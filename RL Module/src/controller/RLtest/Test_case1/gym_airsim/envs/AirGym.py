...
class AirSimEnv(gym.Env):
    ...
    def computeReward(self, now, yaw, check_yaw):
        if check_yaw == True:
            r = 2*math.cos(yaw)*0.2
            return r
        else:
            distance_now = np.sqrt(np.power((now.x_val - self.root[0]),2) + np.power((now.y_val -self.root[1]),2))
    ...
    def _step(self, action):
        ...
        airgym.
        now = airgym.getPosition()
        track = airgym.root_direction(now) 
        yaw = airgym.get_yaw()
        check_yaw = True
        if collided == True:
            done = True
            reward = -100.0
            distance = np.sqrt(np.power((now.x_val - self.root[0]),2) + np.power((now.y_val -self.root[1]),2))
        elif collided == 99:
            done = True
            reward = 0.0
            distance = np.sqrt(np.power((now.x_val - self.root[0]),2) + np.power((now.y_val -self.root[1]),2))
            ...
            reward, distance = self.computeReward(now,yaw, check_yaw)
        if distance > 100:
            done = True
            reward = 100.0
