...
def huber_loss(y, y_hat, delta): # delta = 1.0
    ...
    loss_per_sample = element_select(less(abs_error, delta), less_than, more_than) # less return 1 if left < right else 0
    # if flag != 0 -> less_than
...
def compute_reward(quad_state, quad_vel, collision_info, action, yaw):
    # thresh_dist = 1.5
    # beta = 1
    v = 1
    z = -3
    #test
    # pts1 = [np.array([-20.5, -45.5, z]), np.array([-2.5, -45.5, z]), np.array([-2.5, 4.5, z]), np.array([-20.5, 4.5, z]), np.array([-20.5, -45.5, z])]
    # pts2 = [np.array([-26.5, -55.5, z]), np.array([3.5, -55.5, z]), np.array([3.5, 14.5, z]), np.array([-26.5, 14.5, z]), np.array([-26.5, -55.5, z])]

    
    # quad_pt = np.array(list((quad_state.x_val, quad_state.y_val, quad_state.z_val)))
    # pts = [np.array([z, quad_state.y_val, quad_state.z_val]),np.array([-z, quad_state.y_val, quad_state.z_val])]
    if collision_info.has_collided:
        reward = -100
    else: 
        # reward = v*math.cos(yaw)*0.2 - 0.01
        reward = v*math.cos(yaw)*0.2
        if action == 2:
            reward += 10
    # else:    
    #     dist = 10000000
    #     for i in range(0, len(pts1)-1):
    #         dist = min(dist, np.linalg.norm(np.cross((quad_pt - pts1[i]), (quad_pt - pts1[i+1])))/np.linalg.norm(pts1[i]-pts1[i+1]))
    #     for i in range(0, len(pts2)-1):
    #         dist = min(dist, np.linalg.norm(np.cross((quad_pt - pts2[i]), (quad_pt - pts2[i+1])))/np.linalg.norm(pts2[i]-pts2[i+1]))
    #     print(dist)
...
class RLController:
    def __init__(self):
        ...
        self.__takeoff()
        self.agent = DeepQAgent((NumBufferFrames, SizeRows, SizeCols), NumActions, monitor=True)
        responses = self.client.simGetImages([airsim.ImageRequest(3, airsim.ImageType.DepthPerspective, True, False)])
        self.current_state = transform_input(responses)
    ...
    def control(self):
        ...
        change = interpret_action(action)
        ...
        if (vx == 0 and vy == 0):
            vx = velocity * math.cos(yaw);
            vy = velocity * math.sin(yaw);
        self.client.moveByVelocityZAsync(vx, vy,-2, 1, airsim.DrivetrainType.ForwardOnly, airsim.YawMode(False, 0)).join()
        ...
        reward = compute_reward(quad_state, quad_vel, collision_info, action, yaw)
        done = isDone(reward)
        print('Action, Reward, Done:', action, reward, done)

        self.agent.observe(self.current_state, action, reward, done)
        self.agent.train()
